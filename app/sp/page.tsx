...
import { statusColor, type Ticket, type Profile, type Store } from '@/lib/types';
      ...
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-red-600 to-red-700 rounded-2xl p-8 text-white">
          ...
          <div className="mt-6 flex gap-3 flex-wrap">
            <Link href="/sp/tickets/new" className="bg-white text-red-700 font-semibold px-5 py-3 rounded-lg hover:bg-red-50 flex items-center gap-2"><Plus size={18}/> Raise a query · ప్రశ్న ఇవ్వండి</Link>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <KpiCard icon={<MessageSquare className="text-red-600"/>}   label="Total queries" value={all.length}/>
          <KpiCard icon={<Clock        className="text-orange-600"/>} label="Open"          value={openCount}/>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link href="/sp/tickets" className="text-sm text-red-600 hover:underline">View all →</Link>
          </div>
          <div className="divide-y divide-slate-100">
            ...
            {all.length === 0 && (
              <div className="p-6 text-center text-slate-500">
                No tickets yet. Tap &quot;Raise a query&quot; to start.
                <div className="text-sm mt-1">ఇంకా టికెట్‌లు లేవు. &quot;ప్రశ్న ఇవ్వండి&quot; నొక్కండి.</div>
              </div>
            )}
          </div>
        </div>
