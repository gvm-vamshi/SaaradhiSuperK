...
export default async function AsmTicketDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const ticketId = parseInt(id, 10);
  if (isNaN(ticketId)) notFound();

  const supabase = await createClient();
  // RLS will block if this ticket isn't from the ASM's stores
  ...
  return (
    <Shell title="Area Manager" icon={<Shield size={20}/>} accent="slate">
      <TicketDetail
        ticket={ticket}
        messages={messages || []}
